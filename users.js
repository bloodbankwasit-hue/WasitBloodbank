...
function renderPermCheckboxes(containerId, selected=[]){
  ...
  box.innerHTML = ALL_PERMS.map(p=>`
    <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer;background:#fafafa;border:1px solid #e0e0e0;border-radius:8px;padding:7px 10px">
...
async function loadUsers(){
  ...
  if(data&&data.length){
    G('usersTbl').innerHTML=`<div class="tw"><table><thead><tr>
      ...
      <td><span style='font-size:12px;color:#757575'>${u.role==='admin'?'مدير النظام ✅':((u.permissions||[]).length+' قسم')}</span></td>
      ...
      <td>${u.id!==SES?.user?.id?`
        ...
        </button>`:'<span style="font-size:11px;color:#9e9e9e">حسابك الحالي</span>'}
      </td>
